...
def rasterize_agents(
    ...
    hist_image.scatter_(dim=2, index=raster_hist_pos_flat[:, :, 1:],
                        src=torch.ones_like(hist_image) * -1)  # mark other agents with -1
    hist_image.scatter_(dim=2, index=raster_hist_pos_flat[:, :, [0]],
                        src=torch.ones_like(hist_image))  # mark ego with 1.
    hist_image[:, :, 0] = 0  # correct the 0th index from invalid positions
    hist_image[:, :, -1] = 0  # correct the maximum index caused by out of bound locations
    ...
    if llm_input_ids == None and not reset:
        maps_root_dir = "maps"
        for i in range(b):
            save_image(maps[i], f"{maps_root_dir}/maps_map9_guide_{i}.png")
